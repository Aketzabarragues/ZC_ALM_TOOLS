...
namespace ZC_ALM_TOOLS.Models.Generator
{
    ...
    public class DevicesEnvironment
    {
            ...
            bool validatePlc = true)
        {
            ...
            App.ServiceProvider?.GetService<ZC_ALM_TOOLS.Services.Common.ILogService>()?.Write($"[DEVICES-ENVIRONMENT] [DevicesEnvironment] Validando entorno PLC para '{category.Name}'...");

            // Validar Tabla de Constantes (Configuración Global)
            if (tiaPlcService.FindTagTableByName(settings.TiaTable) == null)
            {
                App.ServiceProvider?.GetService<ZC_ALM_TOOLS.Services.Common.IStatusService>()?.Set($"[DEVICES-ENVIRONMENT] [DevicesEnvironment] Error: No existe la tabla de constantes '{settings.TiaTable}'.", StatusType.Error);
                return;
            }
            ...
            {
                App.ServiceProvider?.GetService<ZC_ALM_TOOLS.Services.Common.IStatusService>()?.Set($"[DEVICES-ENVIRONMENT] [DevicesEnvironment] Error: No existe la tabla de variables '{category.TiaTable}'.", StatusType.Error);
                return;
            }
            ...
            {
                App.ServiceProvider?.GetService<ZC_ALM_TOOLS.Services.Common.IStatusService>()?.Set($"[DEVICES-ENVIRONMENT] [DevicesEnvironment] Error: No existe el bloque '{category.TiaDbName}'.", StatusType.Error);
                return;
            }
